...
def create_intent_analysis_workflow() -> StateGraph:

    workflow = StateGraph(IntentAnalysisState)
    
  
    workflow.add_node("get_chat_history", get_chat_history_node)
    workflow.add_node("recall_candidate_tags", recall_candidate_tags_node)
    workflow.add_node("get_user_tags", get_user_tags_node)
    workflow.add_node("analyze_intent", analyze_intent_node)
    workflow.add_node("update_user_tags", update_user_tags_node)
    
 
    workflow.add_edge(START, "get_chat_history")
    workflow.add_edge("get_chat_history", "recall_candidate_tags")
    workflow.add_edge("recall_candidate_tags", "get_user_tags")
    workflow.add_edge("get_user_tags", "analyze_intent")
    workflow.add_edge("analyze_intent", "update_user_tags")
    workflow.add_edge("update_user_tags", END)
...
def run_intent_analysis(
    ...
    """运行客户意图分析工作流"""
    logger.info(f"开始执行客户意图分析工作流，会话ID: {conversation_id}，会话会话ID: {session_id}，用户ID: {user_id}")
    
    workflow = create_intent_analysis_workflow()
    logger.info(f"意图分析工作流创建完成，准备执行")
    ...
    logger.info(f"客户意图分析工作流执行完成，会话ID: {conversation_id}，用户ID: {user_id}，最终结果: {result}")
    return result
