...
第4个测试会失败，因为deepseek不能在第一次按要求只输出一个节点
...
def db_node(state: AgentState):
    ...
    return {
        "messages": [
            HumanMessage(content=result["messages"][-1].content, name="db_node")
        ]
    }
...
code_agent = create_react_agent(
    llm, 
    tools=[python_repl], 
    state_modifier="通过运行 Python 代码来绘制图表或输出执行结果。"
)
...
def supervisor(state: AgentState):
    system_prompt = (
        ...
        "5. 绝不能连续两次选择同一位工作人员。如果上一条回复来自 “sqler”，则不能再次选择 “sqler” \n"
        "6. 当用户请求有答案或无法满足时，请选择 FINISH。\n"
        "7. 仔细分析对话历史后再做出决定。\n"
        "8. 有的任务需要多个工作节点协同串行完成，请根据任务需求选择合适的工作节点。但每次输出必须选择一个工作节点，不能同时选择多个工作节点。\n"
        "9. 输出必须是严格的 JSON，格式为 {{\"next\": \"<选中的成员>\"}}，候选成员有：chat、graph_kg、vec_kg、db_node、code_node、FINISH，只能选一个\n"
    )
