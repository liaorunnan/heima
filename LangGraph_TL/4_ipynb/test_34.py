...
"""
和上一个脚本类似，只是这个是异步情况下，测试 langgraph 流式输出的方法的一种模式：messages
...
这里测试了messages模式 这个参数，记录每个`messages`中的增量`token`
    ...
    messages：记录每个`messages`中的增量`token`            ！！！该脚本测试的是这个参数
...
print("******************** test01：参数设置为 messages ********************")

from langchain_core.messages import AIMessageChunk, HumanMessage
inputs = [HumanMessage(content="what is the weather in sf")]
first = True
async def stream_function():
    async for msg, metadata in graph.astream({"messages": ["你好，帮我查询一下数据库中北京的天气数据"]}, stream_mode="messages"):
