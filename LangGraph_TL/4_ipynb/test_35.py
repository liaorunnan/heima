...
"""
LangGraph中的事件流
astream_events 是 langgraph 流式输出的方法的一种模式：astream_events
两个测试：
    测试 astream_events 输出格式，
    提取 AIMessageChunk 的内容（包括模型的工具响应tool_calls和内容响应content）
...
async def stream_function():
    async for event in graph.astream_events({"messages": ["北京的天气怎么样"]}, version="v2"):
        kind = event["event"]
...
print("\n")
print("\n")
print("******************** test02：astream_events 模式提取 AIMessageChunk ********************")
        ...
        if kind == "on_chat_model_stream":
            ...
            # 输出工具调用信息（友好格式）
                ...
                for tool_call in chunk.tool_calls:
                    if tool_call.get('name'):  # 只有完整的工具调用才显示
                        print(f"\n[调用工具: {tool_call['name']}]\n", end="", flush=True)
