...
print("\n\n\n")
# 运行图，直至到断点的节点
print("*********************** 第一次：删除 muyu 的信息 ***********************")
for chunk in graph.stream({"user_input": "我将在数据库中删除 id 为 muyu 的所有信息"}, config,  stream_mode="values"):
    print(chunk)
    print("-" * 10)
...
a = graph.update_state(config, snapshot.values)
print(a)

print("=" * 50)
print("\n\n\n")

print("*********************** 第三次：设置输入为 None，继续执行后续的操作 ***********************")
# 修改完状态后，如果想让图基于`breakpoint`继续执行后续的操作，则只需要在`stream`方法中将`input`参数设置为 `None`, 
# 则会让图形从上次中断的地方继续
...
print("*********************** 第四次：同一进程下，删除 muyu2222 的信息 ***********************")
# 运行图，直至到断点的节点
for chunk in graph.stream({"user_input": "我将在数据库中删除 id 为 muyu2222 的所有信息"}, config,  stream_mode="values"):
    print(chunk)
    print("-" * 10)
print("=" * 50)
print("\n\n\n")
...
print("*********************** 第六次：同一进程下，设置输入为 None，继续执行后续的操作 ***********************")
for chunk in graph.stream(None, config, stream_mode="values"):
    print(chunk)    
    print("-" * 10)
...
print("*********************** 第七次：非敏感操作，直接执行后续的操作 ***********************")
# 运行图，直至到断点的节点
for chunk in graph.stream({"user_input": "你好，请你介绍一下你自己"}, config,  stream_mode="updates"):
    print(chunk)
    print("-" * 10)
print("=" * 10)
print("\n")
