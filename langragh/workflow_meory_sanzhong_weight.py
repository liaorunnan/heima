...
# 导入向量数据库操作类
from workflow_db_yiwang_weight import CoreMemory, ArchivalMemory, ChatLog
...
try:
    ...
    print(f"核心记忆索引已存在或初始化失败: {e}")

try:
    ArchivalMemory.init()
    print("归档记忆索引已初始化")
except Exception as e:
    print(f"归档记忆索引已存在或初始化失败: {e}")

try:
    ChatLog.init()
    print("对话日志索引已初始化")
except Exception as e:
    print(f"对话日志索引已存在或初始化失败: {e}")
...
def search_archival_memory(query: str, thread_id: str):
    ...
    # 使用向量检索归档记忆
    results = archival_memory_db.search_by_vector(thread_id, query_vector, k=10,decay_rate=0.1)
    ...
    return "\n".join([f"{r.content} (ID: {r.id}, 时间: {r.created_at})" for r in results])

@tool
def delete_duplicate_memory(id_list: str):
    """根据ID列表删除归档记忆"""
    id_list = id_list.split(",")
    print(f"【删除重复记忆工具】: id_list = {id_list}")
    for id in id_list:
        archival_memory_db.delete_by_id(id.strip())
    return "已删除指定ID的归档记忆。"
...
def call_model(state: AgentState):
    ...
    system_prompt = (
        ...
        "4. `delete_duplicate_memory`: 删除重复的归档记忆。\n"
        "请根据对话内容自动判断是否需要操作记忆：\n"
        "- 如果核心记忆中已有答案，不必再查别的记忆\n"
        "- 如果在记忆中找到答案，绝对不要再次添加到归档记忆中\n"
        "- 只有当信息是新的且值得记住时，才使用 add_archival_memory 工具\n"
        "- 如果发现记忆有重复或者矛盾，根据时间顺序保留最新的记忆，删除之前时间的记忆\n"
        "5. 如果需要删除重复记忆，使用 delete_duplicate_memory 工具。\n"
        "注意：调用工具时（除了 delete_duplicate_memory），必须传入当前的 thread_id。\n"
        "删除delete_duplicate_memory的时候，需要传入要删除的记忆ID列表，多个ID之间用逗号分隔。id和时间从search_archival_memory中获取。"
        "【记忆管理策略】\n"
        "- **回溯权重**：检索到的归档记忆如果时间很久远，且与当前情况冲突，请以当前对话为准，并考虑删除旧记忆。\n"
        "- **记忆升级**：如果你发现检索到的归档记忆里包含用户的身份信息（例如检索到'ID:123 内容:我是一名医生'），而当前核心画像里没有，请立即调用 `update_core_memory` 补充进去。\n"
        "- **去重**：如果找到答案，不要再次调用 add_archival_memory。"
    )
