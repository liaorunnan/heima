...
client = OpenAI(
    base_url=settings.refin_emb_url.replace('/models', ''),
    api_key=settings.Emb_token  # vLLM 默认不需要认证，但需要提供任意值
)

def RefineEmbedding(text, model="gte-Qwen2"):
...
def test_vllm_connection():
    try:
        response = requests.get(settings.refin_emb_url, timeout=5)
...
if __name__ == '__main__':
    ...
    embedding = RefineEmbedding(text)
