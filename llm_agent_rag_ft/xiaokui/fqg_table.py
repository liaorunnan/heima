...
class Wendadui(Document):
    query = Text(analyzer='smartcn')
    answer= Text(analyzer='smartcn')
    ...
    @classmethod
    def query(self, text):
        items = self.search().query("match", query=text)[:10].execute()

        return [FAQItem(id=item.meta.id, query = item.query,answer=item.answer) for item in items]
