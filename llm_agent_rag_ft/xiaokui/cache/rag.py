...
from xiaokui.index_rank.llm import chat
from xiaokui.index_rank.embedding import get_embedding
from xiaokui.index_rank.indexing import VecIndex
from xiaokui.fqg_table import Wendadui
from xiaokui.index_rank.ranker import rank

from redis_om import Migrator
from cache import QA
...
while True:
    ...
    answer = QA.find(QA.query == query).all()
    ...
    FAQ = Wendadui.query(query)

    if FAQ:

        print("FAQ的结果：", FAQ[0].answer)
        andrew = QA(query=query, answer=FAQ[0].answer)
        andrew.save()
        andrew.expire(120)
        print("*" * 30)
        continue
    ...
    print("回答：", answer)
    print("*" * 30)
    andrew = QA(query=query, answer=answer)
