...
from rag.items import MuilPhotoItem
from Multimodal.Multimodal_photo import Multimodal_Photo
...
class Multimodal_Photo(Document):
    photo_ver = Text(analyzer="smartcn")
    photo_name = Text(analyzer="smartcn")
    photo_meta = Keyword()

    class Index:
        name = 'MultimodalPhoto'
        settings = {
            "number_of_shards": 2,
        }
    ...
    def query(self, item,type=''):
        ...
        items = self.search().query("match", photo_name=item)[:10].execute()
        return [Multimodal_Photo(id=item.meta.id, photo_ver=item.photo_ver, photo_name=item.photo_name, photo_meta=item.photo_meta) for item in items]
...
if __name__ == '__main__':
    num = 1
    Multimodal_Photo.init()
    photo = Multimodal_Photo(meta={'id': num}, photo_ver=item['photo_ver'], photo_name=item['photo_name'], photo_meta=item['photo_meta'])
    photo.save()

    exit()
    items = Multimodal_Photo.scan_moban()
    for item in items:
        print(item.child)
        exit()
